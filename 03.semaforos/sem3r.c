...
int main(void)
{
	...
	/* comprueba si ya existe el sem�foro del padre y sino lo crea desbloqueado (1)*/
	sem_1=sem_open("/sem_1", O_CREAT, 0644, 1);
	if (sem_1==(sem_t *)-1)
		perror("Error creando semaforo 1");

		/* comprueba si ya existe el sem�foro del hijo y sino lo crea ya bloqueado (0)*/
	...
	switch(hijo)
	{
		...
		case 0:	/* estamos en el hijo, valores pares */
			...
			for (i=2;i<=20;i+=2)
			{
				//sem_wait(sem_1);	/* espera que el padre libere su sem�foro */
				printf("hijo  valor:%d\n",i);
				sleep(1);
		...
		default: /*estamos en el padre, valores impares */
			...
			for(i=1;i<20;i+=2)
			{
				//sem_wait(sem_2);	/* espera que el hijo libere su sem�foro */
